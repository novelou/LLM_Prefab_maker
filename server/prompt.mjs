...
When revising, regenerate the complete source by default and preserve parts the user did not ask to change. Use line edits only for small, localized changes. Use supplied images as evidence of what is visible; repair occlusion, winding and connections rather than merely renaming parts. When repairing an error, preserve correct parts of the source.
...
export function messagesFor({ prompt, source, images = [], error, editFeedback, seed }) {
  const editFormat = `Line-edit format: return ONLY a JSON object with {"mode":"edits","edits":[{"startLine":1,"old":"complete original line(s)","new":"replacement line(s)"}]}. Use one-based line numbers. Copy complete old lines exactly, including indentation, without the number and | prefix. Each startLine refers to the original numbered source; repeated old text on other lines is allowed. Keep edits small and use no more than 32. An empty new string deletes the specified lines. Use JSON string escaping and no markdown fences or explanation.`;
  const text = source
    ? `${error ? `Repair this source. Error: ${error}` : 'Revise this source according to the additional user instruction.'}${editFeedback ? `\nPrevious edit response failed; no changes were applied. ${editFeedback}` : ''}\nNumbered current source (the number and | are not code):\n${numberedSource(source)}\n\n${error ? `Prefer the JSON line edits below when they can correct the error without changing other parts. If the fix needs broader changes, return the full corrected JavaScript source as plain text. ${editFormat}` : `Regenerate and return the complete revised JavaScript source as plain text by default, with no markdown or explanation. You may restructure code and geometry as needed to satisfy the instruction while preserving unrelated features. Choose line edits only when the change is clearly small and localized to a few existing lines; for new parts or coordinated changes, return the full source. ${editFormat}`}\nSeed: ${seed}\nUser request: ${prompt}`
    : `Seed: ${seed}\nUser request: ${prompt}`;
  return [
