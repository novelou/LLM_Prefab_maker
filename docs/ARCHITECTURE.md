...
| `POST /api/generate` | prompt、seed、任意のsource / images / error / editFeedback / streamOutput | 通常は完全なsourceをJSONで返す。streamOutput時は生成差分と結果・エラーをNDJSONで返す |
| `POST /api/cancel`   | `{}`                                                                      | 進行中の上流リクエストを中止                                                         |

エラーはJSONの `error` と `code` で返します。認証・レート制限・通信・タイムアウト・形式・未完了・実行契約の違反を区別します。完全な契約違反ソースは、1回の修復のため `details.rejectedSource` に返すことがあります。追加修正はソース全文の再生成を基本とし、小さな変更だけ部分編集を選べます。自動修復では局所的な部分編集を優先します。部分編集はサーバーが指定行の元テキストと照合して適用し、完成したソースを返します。編集形式や照合が不正な場合は `patch` エラーにし、フロントエンドは元コードと失敗理由を `editFeedback` で渡して各段階の編集応答を最大3回試します。編集適用後にコード検証が失敗した場合は、自動修復が有効なら1回修復し、その後も失敗すれば終了します。プロバイダーのreasoning本文はUIへ返しません。
